...
module.exports = function (css) {
  ...
  function mixin(expression) {
    const split = expression.split(':'); // Remove leading and trailing spaces
  ...
  mixin.all = function (rules) {
    return css.apply(null, [new Array(rules.length + 1).fill('')].concat(rules.map(rule => mixin(rule))));
  };
